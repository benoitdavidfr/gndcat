...
/** gestion des requêtes Http avec les options, version 1 pour mémoire */
class HttpV1 {
  ...
  /** Exécute une appel HTTP et retourne un array constitué de 2 champs 'headers' et 'body'.
   *
   * En cas d'erreur:
   *  - si l'option ignore_errors n'est pas définie à true alors lève une exception (par défaut)
   *  - si l'option ignore_errors est définie à true alors retourne l'erreur dans le headers
   *    et généralement le message d'erreur dans le body.
   *    Dans ce cas, le headers peut être analysé avec errorCode() pour obtenir le code d'erreur Http.
   *
   ...
   *
   * @param array<string,mixed> $options les options de l'appel, voir ci-dessus les valeurs possibles.
   */
  static function call(string $url, array $options=[]): array {
    //echo "Http::call($url)\n";
    $nbretries = $options['max-retries'] ?? 0; // nombre de relance à effectuer en cas d'erreur de timeout
    echo "nbretries=$nbretries<br>\n";
    unset($options['max-retries']);
    ...
    if ($body !== false) // retour si !erreur || (ignore_errors && !timeout)
      return [
        'headers'=> $http_response_header,
        'body'=> $body,
      ];
    
    // Reste: erreur && (timeout || !ignore_errors)
    // retour d'un résultat ou lancement d'une alerte
    if (!($options['ignore_errors'] ?? false)) { // si !ignore_errors alors lancement exception 
      throw new Exception("Erreur '".($http_response_header[0] ?? 'timeout')."' dans Http::call() : sur url=$url");
    }
    
    // Le dernier cas est celui timeout && ignore_errors
    // construction d'une erreur similaire aux autres pour facilier son analyse
    return [
      'headers'=> [
  ...
  /** analyse les en-têtes et retourne le code d'erreur HTTP ou -2 si ce code n'est pas trouvé.
   *
  ...
  /** extrait des headers le Content-Type */
 ...
 */
class Http {
  static array $lastHeaders=[]; // headers retournés par le dernier appel
...
if (0) { // Test V1
  switch ($_GET['action'] ?? null) {
    case null: { // appel pour fournir le menu de tests
      echo HTML_HEADER,"<b>Menu de tests utitaires de la classe Http</b><ul>\n";
      ...
      echo "<li><a href='?action=404Call'>Appel erreur 404 sans ignore_errors -> une exception est lancée</a></li>\n";
      echo "<li><a href='?action=404Callwie'>Appel erreur 404 avec ignore_errors -> retourne le code d'erreur</a></li>\n";
      echo "<li><a href='?action=timeOutCall'>Appel timeout sans ignore_errors -> une exception est lancée</a></li>\n";
      echo "<li><a href='?action=timeOutCallwie'>Appel timeout avec ignore_errors -> retourne le code d'erreur timeout</a></li>\n";
      ...
      echo "<li><a href='?action=timeOutCallMR3'>Appel timeout sans ignore_errors -> une exception est lancée</a></li>\n";
      echo "<li><a href='?action=timeOutCallwieMR3'>Appel timeout avec ignore_errors -> retourne le code d'erreur timeout</a></li>\n";
      echo "</ul>\n";
      die();
    }
    case 'simpleCall': {
    ...
    case '404Callwie': {
      $url = selfUrl().'?action=404';
      echo "url=$url<br>\n";
      $result = Http::call($url, ['ignore_errors'=> true]);
      break;
    }
    case 'timeOutCall': {
    ...
    case 'timeOutCallwie': {
      $url = selfUrl().'?action=timeOut';
      echo "url=$url<br>\n";
      $result = Http::call($url, ['timeout'=> 5, 'ignore_errors'=> true]);
      break;
    }
    case 'timeOutCallMR3': {
    ...
    case 'timeOutCallwieMR3': {
      $url = selfUrl().'?action=timeOut1/2';
      echo "url=$url<br>\n";
      $result = Http::call($url, ['max-retries'=> 3, 'timeout'=> 5, 'ignore_errors'=> true]);
      break;
    }
    case 'serverCall': {
  ...
  echo "<table border=1>",
       "<tr><td colspan=2><pre>"; print_r($result); echo "</td></tr>",
       "<tr><td>errorCode</td><td><pre>",Http::errorCode($result['headers']),"</td></tr>",
       "<tr><td>contentType</td><td><pre>",Http::contentType($result['headers']),"</td></tr>";
  if (Http::contentType($result['headers']) == 'application/json')
    echo "<tr><td>JSON</td><td><pre>",Yaml::dump(json_decode($result['body'], true)),"</pre></td></tr>\n";
  echo "</table>\n";
}
